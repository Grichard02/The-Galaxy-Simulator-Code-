...
def detect_prob(disc_vol_scaled):
    ...
    if ratio < 0.002: # set very low ratios to probability of 0 
        detection_prob = 0 
    elif ratio >= 5: # set high ratio to probability of 1 
        detection_prob = 1 
    else:
        detection_prob = .004757 * ratio**4 + 0.06681 * ratio**3 - 0.3605 * \
...
def spirals(b, r, rot_fac, fuz_fac, arm):
    ...
    for i in range(theta_max_degree): # range(0,600,2) for no black hole 
...
def star_haze(disc_radius_scaled, density):
    ...
    density = multipier to vary number of stars posted
    """
